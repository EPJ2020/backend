...
public class MatcherService {
  ...
  public List<Appgroup> calculateSuggestionsForUser(Appuser user) {
    ...
    for (Appgroup group : groups) {
      //TODO Rewrite according to Issue 84
      if (sharesAnElement(group.getTags(), user.getTags())
          && notAlreadySwipedUser(user.getUserId(), group.getGroupId())
          && group.getActive()) {
        result.add(group);
      }
    }
  ...
  public List<Appuser> calculateSuggestionsForGroup(Appgroup group) {
    ...
    for (Appuser user : users) {
      //TODO Rewrite according to Issue 84
      if (sharesAnElement(user.getTags(), group.getTags())
          && notAlreadySwipedGroup(user.getUserId(), group.getGroupId())
          && user.getActive()) {
        result.add(user);
      }
    }
  ...
  private boolean notAlreadySwipedUser(long userId, long groupId) {
    ...
    for (Match m : matches) {
      //TODO Rewrite according to Issue 84
      if (m.getGroup().getGroupId() == groupId
          && m.getUser().getUserId() == userId
          && m.getUserAccept() == null) {
        return true;
      }
    }
    return false;
